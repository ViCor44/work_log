...
{
  String xml_parameters = "<?xml version = \"1.0\" ?><inputs><poolName>Disco River</poolName><freeChlorine>" + String(freeChlorine, 2) + "</freeChlorine><pH>" + String(pH, 2) + "</pH><temperature>" + String(temperature, 1) + "</temperature><alarme>" + String(alarm) + "</alarme></inputs>";
...
void loop() {
  freeChlorine = analogRead(0)/178.0;
  pH = analogRead(1)/62.57;
  temperature = analogRead(2)/17.68;
