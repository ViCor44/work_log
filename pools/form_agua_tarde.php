...
$sql = "
    ...
    FROM
        tanks t
    WHERE
        t.water_reading_frequency > 1 -- Apenas tanques com 2 leituras/dia
    ORDER BY
        t.type, t.name ASC;
";
...
foreach ($all_tanks as $tank) {
    if ($tank['name'] === 'Rede' || $tank['name'] === 'Agua Quente Edificio') {
        $special_tanks[] = $tank;
...
<div class="container-fluid mt-4">
    <div class="d-flex align-items-center mb-4">
        ...
        <h1 class="h3 mb-0 ms-3">Registar Leituras da Tarde</h1>
    </div>

    <form action="guardar_registos_batch.php" method="POST" id="water-form-tarde">
        <input type="hidden" name="tipo_registo" value="agua_tarde">
        ...
        <?php if(count($all_tanks) > 0): ?>
            <div class="form-actions text-end mt-4">
                ...
                <button type="submit" class="btn btn-primary">Guardar Leituras da Tarde</button>
            </div>
        <?php else: ?>
            <div class="alert alert-warning">Não existem tanques configurados para uma segunda leitura diária.</div>
        <?php endif; ?>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('water-form-tarde');
    if (!form) return;
