...
if (!in_array($pagina_atual, $paginas_permitidas)) {
    ...
    if (!$licenca_valida) {
        // Se for chamada API (ficheiro dentro de /api/), devolve erro JSON
        if (strpos($_SERVER['PHP_SELF'], '/api/') !== false) {
            header('Content-Type: application/json; charset=utf-8');
            http_response_code(403);
            echo json_encode(['error' => 'Licença inválida ou expirada']);
            exit;
        } else {
...
if (isset($_SESSION['user_type']) && $_SESSION['user_type'] === 'viewer') {
    ...
    $allowed_pages_for_viewer = [
        ...
        'analise_pid_metricas.php', // Página de análise PID com métricas avançadas
        ...
        'dynamic_setpoint_config.php', // API de leitura de SP dinâmico (viewer apenas leitura)
